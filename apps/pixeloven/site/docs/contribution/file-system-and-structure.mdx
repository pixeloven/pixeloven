...
It should be noted that we use both `.ts` and `.tsx`. The general rule is any file that has **_JSX_** should use the `.tsx` extension. Everything else should be `.ts`. The `.js` and `.jsx` extensions are not supported as this is a pure TypeScript environment.
