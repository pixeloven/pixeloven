...
export default (context: PixelOvenRunContext) => {
    const typeDoc = async (args: string[] = []) => {
        ...
        if (typedocConfigPath) {
            ...
            print.warning(`Unable to find "${typedocFileName}" reverting to default configuration`);
        }
        ...
        if (tsconfigConfigPath) {
            ...
            print.warning(`Unable to find "${tsconfigConfigPath}" reverting to default configuration`);
        }
        if (typedocConfigPath && tsconfigConfigPath) {
            return pixeloven.runBin("typedoc", [
                "--options",
                typedocConfigPath,
                "--tsconfig",
                tsconfigConfigPath
